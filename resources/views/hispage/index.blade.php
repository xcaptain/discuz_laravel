  ...
  <div class="my-qz-main content-grid">
    <div class="his-qz-l person-wrap grid-7">
      ...
      <div class="person-nav">
        <ul>
          <li{if $type == 'reply'} class="on"{/if}>
            <a href="hispage-$_GET[uid]-0-1.html">
              <i class="reply-icon"></i>
              {$usersign}参与的话题
            </a>
          </li>
          <li{if $type == 'post'} class="on"{/if}>
            <a href="hispage-$_GET[uid]-1-1.html">
      ...
      <div class="zhuti-list-box" id="J-tiezi-list">
          ...
          <!--{eval $authornick = empty($nicknames[$thread[authorid]]) ? $thread[author] : $nicknames[$thread[authorid]]}-->
          <!--{eval $lastposternick = empty($nicknames[$thread[lastposterid]]) ? $thread[lastposter] : $nicknames[$thread[lastposterid]]}-->
        <div class="list-list">
          <div class="list-list-l{if empty($thread[message])} list-l-nop{/if}">
            <h3>
              <a href="thread-$thread[tid]-1-1.html" target="_blank">{echo messagecutstr($thread[subject], 80)}</a>
              <!--{if $thread[digest]}-->
            ...
            <p>
              <!--{if empty($thread[img])}-->
              <!--{eval echo messagecutstr($thread[message], 200);}-->
              <!--{else}-->
              <!--{eval echo messagecutstr($thread[message], 100);}-->
              <!--{/if}-->
            </p>
            <!--{/if}-->
            <!--{if $thread[img]}-->
            <div class="list-img-box">
              <ul class="list-img  J-list-img">
                <!--{loop $thread[img] $k $img_url}-->
                <!--{if $k < 4}-->
                <li>
                  <a class="mutual" href="thread-$thread[tid]-1-1.html" target="_blank"><img src="http://www.7k7kjs.cn/zeze/img/common/empty.png" data-src="$img_url[small]" data-big="$img_url[big]" class="lazy-img"/><div class="shade"></div></a>
                </li>
                <!--{/if}-->
                <!--{/loop}-->
              </ul>
            ...
            <div class="forum-name">
              <a class="forum-color01" href="forum-$thread[fid]-1.html" target="_blank">
                <span class="forum-text">$thread[forumname]<i></i></span>
              </a>
            </div>
          </div>
          <div class="list-list-r">
            <div class="list-list-rt">
              <div class="list-user">
                <a href="hispage-$thread[authorid]-0-1.html" target="_blank">$authornick</a>
              </div>
              <div class="list-user-lang">
                <span>$lastposternick</span>
              </div>
              <div class="list-zan-box">
                <div class="list-zan-l">
                  <div class="list-zan-r">
                    <div class="list-zan-txt">赞</div>
                    <div class="list-zan-num">$thread[recommend_add]</div>
                  </div>
                </div>
              </div>
